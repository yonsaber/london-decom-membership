...
RSpec.feature 'Membership Code Admin', type: :feature do
  scenario 'should list existing codes, and can generate more low income codes' do
  ...
  scenario 'should list existing codes, and can generate more membership codes' do
  ...
  scenario 'should list existing codes, and can generate more direct sale codes' do
    # 12 membership codes are generated in rails_helper.rb, one is automatically assigned to the test user
    stub_eventbrite_event
    login(admin: true)
    visit admin_codes_path
    expect(page).to have_text('There are 0 codes left')
    expect(page).to have_text('No codes have been assigned')
    fill_in 'direct-sale-code-number', with: '2'
    within '#direct-sale-code' do
      click_button('Generate')
    end
    expect(page).to have_text('There are 2 codes left')
    expect(page).to have_text('No codes have been assigned')
  end
