class Admin::LowIncomeRequestsController < AdminController
  def index
    ...
    respond_to do |format|
      format.html
      format.csv { send_data LowIncomeRequest.all.to_csv, filename: "low-income-requests-#{Time.zone.today}.csv" }
    end
  end

  def approve
    @low_income_request = LowIncomeRequest.find(params.expect(:id))
